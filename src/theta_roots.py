...
class ThetaRootsData(ThetaDataBase):
    def __init__(self, log_level: str = "WARNING", output_dir: str = "./") -> None:
        """
        Initialize the ThetaRootsData class.
    ...
    def _process_response(
        ...
        if response:
            ...
            if write_csv:
                self._write_csv(df, type, subtype)
    ...
    def _write_csv(
        ...
        filename = f"{type}_{subtype}.csv"
        filepath = os.path.join(self.output_dir, filename)
        df.to_csv(filepath, index=False)
        self.logger.info(f"CSV file written: {filepath}")

    def get_roots(
        self, asset: str, write_csv: bool = False
    ) -> pd.DataFrame | None:
        """
        ...
        """
        self.logger.info(
            f"Getting root report for {asset}"
        )
        endpoint = f"/v2/list/roots/{asset}"
        params = {}
